...
df, words, toks, ara, pos = upload(filename = "\\tarc.tsv", path = path)
...
def findCompounds(ara, verbose=True):
...
def starter(Composition=False, PP= False, codeSw = False):
    
    # To get the list of percentages of the various realizations
    if PP: 
        if codeSw:
            p1, p2, p3, p4, p5, p6, idx = findCompounds(ara, verbose = False)
        else: 
            p1, p2, p3, p4, p5, p6, idx = findCompounds(ara, verbose = True)
    ...
    if Composition: 
        print("-------------------\n| REFERENCE TABLE |\n-------------------\n pp1=>[PREP+N]\n pp2=>[PREP N]\n pp3=>[PREP+DET+N]\n pp4=>[PREP+DET N]\n pp5=>[PREP DET+N]\n pp6=>[PREP DET N]\n")
        compound = input("Enter the pattern of which you want to observe the composition following the above reference table.\n ex.: write 'pp6'\n right here =>")        
        pp1, pp2, pp3, pp4, pp5, pp6, _ = findCompounds(toks, verbose= False) 
        if compound == 'pp6':
            compound = pp6
        elif compound == 'pp5':
            compound = pp5
...
#to observe the PP schemes percentages run the following command:
#p1, p2, p3, p4, p5, p6, idxs = starter(PP=True)

#To observe the internal composition run the following command: 
#pp1, pp2, pp3, pp4, pp5, pp6, _ = starter(Composition=True)
